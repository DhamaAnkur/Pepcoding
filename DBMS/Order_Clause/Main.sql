...
------------------------------------- Use of order by clause to order the attribute -------------------------------------------------------------------------------------------

select * from employee e order by e.fname ; --- to output a table by an order that's why we used ( order by ) keyword

select * from employee e order by e.super_ssn desc , e.fname desc ; --for check the 


---------------------------------------------------------------(1)Question --> Retrieve a list of employee and the project they are working on ordered by department , 
...
---------------------------------------------------------------(2)Question --> Retrieve a list of employee and the project they are working on ordered by department , 
---- and within each department , ordered desceasing by d.dname  , alphabetically by last name then first name.


select e.fname , e.lname , p.pname 
from employee e  , project p  , works_on wo , department d 
where e.ssn = wo.essn  and p.pnumber  = wo.pno and d.dnumber  = p.dnum  
